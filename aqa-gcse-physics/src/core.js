...
/* ---------- Physical constants (values used in AQA GCSE papers) ---------- */
const K = { g: 9.8, c: 3.00e8, e: 1.60e-19, sound: 330 };
...
/* ---------- Storage ---------- */
const STORE_KEY = 'livewire.aqa.v1';
const S = (() => {
  let d = {};
  try { d = JSON.parse(localStorage.getItem(STORE_KEY) || '{}') || {}; } catch (e) { d = {}; }
  d.xp ??= 0; d.topics ??= {}; d.badges ??= []; d.settings ??= { sound: true }; d.settings.tier ??= 'H'; d.settings.course ??= 'sep'; d.streak ??= { last: '', n: 0 }; d.days ??= {}; d.games ??= {}; d.log ??= [];
  return d;
})();
/* ---------- Scope: tier (F/H) and course (separate Physics 8463 or Combined Science) ----------
   Items may carry flags: ht = Higher tier only, po = Physics (separate science) only. */
const inScope = (o) => { if (!o) return true; const f = typeof o === 'string' ? o : ((o.ht ? 'ht' : '') + (o.po ? 'po' : ''));
  if (f.includes('ht') && S.settings.tier === 'F') return false; if (f.includes('po') && S.settings.course === 'comb') return false; return true; };
...
const LEVEL_NAMES = ['Spark', 'Current', 'Circuit', 'Conductor', 'Magnet', 'Motor', 'Generator', 'Transformer', 'Power station', 'National Grid', 'Satellite', 'Planet', 'Star', 'Galaxy', 'Universe'];
