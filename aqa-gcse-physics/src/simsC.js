...
SIMS.induction = {
  ...
  draw(c, W, H, st, C) {
    ...
    const gx = cx + 130, gy = 42; CV.line(c, cx - 50, cy - 46, cx - 50, gy, '#B87333', 1.5); CV.line(c, cx - 50, gy, gx - 26, gy, '#B87333', 1.5); CV.line(c, cx + 50, cy - 46, cx + 50, gy + 14, '#B87333', 1.5); CV.line(c, cx + 50, gy + 14, gx - 24, gy + 14, '#B87333', 1.5); CV.circle(c, gx, gy, 26, C.surface, C.ink, 1.5); const ang = clamp(st.emf * 0.5, -1.2, 1.2); CV.line(c, gx, gy + 16, gx + Math.sin(ang) * 22, gy + 16 - Math.cos(ang) * 22, C.bad, 2.2); CV.mono(c, '0', gx, gy - 17, C.muted, 9, 'center'); CV.text(c, 'meter', gx, gy + 36, C.muted, 10, 'center');
